...
# TODO: check later if necessary to optimize to accelerate
def param_random_generator(param_range: dict, mp_num: int = 1):
    """
    ...
    var_dict = {}
    if mp_num > 1:
        for process_num in range(1, mp_num + 1):
            for key, (low, high) in param_range.items():
                v = _sample_value(float(low), float(high))
                var_dict[f"{key}_{process_num}"] = f"{v:.5e}"  # change to `v` if you prefer floats
    else:
        for key, (low, high) in param_range.items():
            v = _sample_value(float(low), float(high))
