...
class XStandardizer:
    def __init__(self, mode='global'):
        ...
        self.mode = mode # 'pointwise' (old) or 'global' (new) or 'channel'

    def fit(self, x_train: np.ndarray, iv_len: int = 0):
        # x_train: [N, Features]
        if self.mode == 'pointwise':
            self.mean = x_train.mean(axis=0)
            std = x_train.std(axis=0)
        elif self.mode == 'global':
            # Compute one scalar mean/std for the entire dataset (preserves relative shape perfectly)
            self.mean = np.mean(x_train)
            std = np.std(x_train)
        elif self.mode == 'channel_wise' and iv_len > 0:
            # Recommended: One stat for IV, one stat for Gm
            # This is best because IV and Gm have different units (A vs S)
            x_iv = x_train[:, :iv_len]
            x_gm = x_train[:, iv_len:]
            
            m_iv, s_iv = np.mean(x_iv), np.std(x_iv)
            m_gm, s_gm = np.mean(x_gm), np.std(x_gm)
            
            # Broadcast to shape
            self.mean = np.concatenate([np.full(iv_len, m_iv), np.full(x_train.shape[1]-iv_len, m_gm)])
            std = np.concatenate([np.full(iv_len, s_iv), np.full(x_train.shape[1]-iv_len, s_gm)])
        else:
            # Fallback to global
            self.mean = np.mean(x_train)
            std = np.std(x_train)

        # Broadcast scalar to array if needed for compatibility
        if np.isscalar(self.mean):
            self.mean = np.full(x_train.shape[1], self.mean)
            std = np.full(x_train.shape[1], std)
...
def load_and_prepare_dual(data_path: str, cfg, PARAM_NAMES, PARAM_RANGE):
    ...
    # FIX: Use 'channel_wise' to preserve shape for CNN
    x_scaler = XStandardizer(mode='channel_wise') 
    x_scaler.fit(X_concat[tr_idx], iv_len=int(np.prod(iv_shape)))
