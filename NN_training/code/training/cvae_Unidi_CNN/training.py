...
def train_one_epoch_dual(
    model, loader, optimizer, scaler, device, meta,
    ...
    meter = dict(
        ...
        phys_sim_loss=0.0, phys_meas_loss=0.0,
    ...
    for x_iv, x_gm, y in loader:
        ...
        with torch.amp.autocast('cuda', enabled=(device.type=='cuda')):
            ...
            if (proxy_iv is not None) and( proxy_gm is not None) and (lambda_cyc_sim > 0):
                ...
                cyc_sim_iv_post, cyc_sim_gm_post, xhat_curr_std = NormCalc_cyc_dual(
                ...
                # plus: Physics Feature Loss Injection
                # temporary hard-coded
                lambda_phys_sim_iv = 0.5
                lambda_phys_sim_gm = 0.1
                lambda_phys_sim = 0.3
                phys_sim_loss = 0.0

                if phys_limit:
                    pred_iv_sim_part = xhat_curr_std[:, :L_iv]
                    target_iv_sim_part = x_flat_std[:, :L_iv]

                    pred_gm_sim_part = xhat_curr_std[:, L_iv:]
                    target_gm_sim_part = x_flat_std[:, L_iv:]

                    phys_loss_sim_iv = calc_advanced_physics_loss(
                        pred_iv_sim_part, target_iv_sim_part, 
                        shape=meta["iv_shape"], 
                        mode='iv'
                        )
                    phys_loss_sim_gm = calc_advanced_physics_loss(
                        pred_gm_sim_part, target_gm_sim_part, 
                        shape=meta["gm_shape"], 
                        mode='gm'
                        )
                    phys_sim_loss = phys_loss_sim_iv*lambda_phys_sim_iv + phys_loss_sim_gm*lambda_phys_sim_gm
            ...
            if proxy_iv and proxy_gm and meas_iter and lambda_cyc_meas > 0:
                ...
                # plus: Physics Feature Loss Injection
                # temporary hard-coded
                lambda_phys_meas_iv = 0.5 
                lambda_phys_meas_gm = 0.1
                lambda_phys_meas = 0.1 
                phys_meas_loss = 0.0

                if phys_limit:
                    pred_iv_meas_part = xmh_curr_std[:, :L_iv]
                    target_iv_meas_part = xm_flat_std[:, :L_iv]
                    pred_gm_meas_part = xmh_curr_std[:, L_iv:]
                    target_gm_meas_part = xm_flat_std[:, L_iv:]

                    phys_loss_meas_iv = calc_advanced_physics_loss(
                        pred_iv_meas_part, target_iv_meas_part, 
                        shape=meta["iv_shape"], 
                        mode='iv'
                        )
                    phys_loss_meas_gm = calc_advanced_physics_loss(
                        pred_gm_meas_part, target_gm_meas_part, 
                        shape=meta["gm_shape"], 
                        mode='gm'
                        )
                    phys_meas_loss = phys_loss_meas_iv*lambda_phys_meas_iv + phys_loss_meas_gm*lambda_phys_meas_gm
            ...
            total = (
                ...
                + lambda_phys_sim * phys_sim_loss
                + lambda_phys_meas * phys_meas_loss
            )
        ...
        meter["phys_sim_loss"] += float(phys_sim_loss)
        ...
        if proxy_iv and proxy_gm and meas_iter is not None and lambda_cyc_meas > 0:
            if valid_count > 0:
                ...
                meter["phys_meas_loss"] += float(phys_meas_loss)
    ...
    train_metrics = {
        ...
        "phys_sim": meter["phys_sim_loss"]/n,
        "phys_meas": meter["phys_meas_loss"]/max(1,meter["n_meas"]) if meter["n_meas"]>0 else 0.0,
...
def evaluate_full_dual(
    model, loader, device, meta,
    y_tf=None, 
    proxy_iv=None, proxy_gm=None, L_iv=None,
    lambda_cyc_sim=0.0,
    meas_loader=None, lambda_cyc_meas=0.0,
    weight_iv=1.0, weight_gm=1.0,
    y_tf_proxy=None,
    x_mu_c=None, x_std_c=None, x_mu_p=None, x_std_p=None,
    y_idx_c_from_p=None,
    PARAM_RANGE=None,
        ...
        for x_iv, x_gm, y in loader:
            ...
            if (proxy_iv is not None) and( proxy_gm is not None) and (lambda_cyc_sim > 0):
                ...
                cyc_sim_iv_post, cyc_sim_gm_post, _ = NormCalc_cyc_dual(
                    ...
                    y_idx_c_from_p, cyc_crit,
                    # weight_iv, weight_gm
                    1.0, 1.0 # fake weights for logging onl
                )
                cyc_sim_post = cyc_sim_iv_post + cyc_sim_gm_post
                ...
                if best_of_k > 1 and bok_use_sim:
                     ...
                     cyc_sim_iv_prior, cyc_sim_gm_prior, _ = NormCalc_cyc_dual(
                        ...
                        y_idx_c_from_p, cyc_crit,
                        # weight_iv, weight_gm
                        1.0, 1.0 # fake weights for logging only
                    )
                else:
                ...
                # plus: Physics Feature Loss Injection
                # temporary hard-coded
                lambda_phys_sim_iv = 5.0 
                lambda_phys_sim_gm = 1.0
                lambda_phys_sim = 2.0 
                phys_sim_loss = 0.0

                if phys_limit:
                    pred_iv_sim_part = x_hat_std_sim_prior[:, :L_iv]
                    target_iv_sim_part = x_flat_std[:, :L_iv]

                    pred_gm_sim_part = x_hat_std_sim_prior[:, L_iv:]
                    target_gm_sim_part = x_flat_std[:, L_iv:]
            ...
            if proxy_iv and proxy_gm and meas_iter and lambda_cyc_meas > 0:
                ...
                # plus: Physics Feature Loss Injection
                # temporary hard-coded
                lambda_phys_meas_iv = 5.0 
                lambda_phys_meas_gm = 1.0
                lambda_phys_meas = 1.0 
                phys_meas_loss = 0.0
    ...
    metrics = {
        ...
        "val_phys_sim": meter["phys_sim_loss"],
        "val_phys_meas": meter["phys_meas_loss"],
        ...
        "val_total_prior": (
            ...
            + lambda_phys_sim * (meter["phys_sim_loss"]/n)
            + lambda_phys_meas * ((meter["phys_meas_loss"]/max(1,meter["n_meas"])) if meter["n_meas"]>0 else 0.0)
        ),
    }
    return metrics
