...
def train_one_epoch(model, loader, optimizer, scaler, device,
    ...
    for x, y in loader:
        ...
        with torch.autocast(device_type='cuda' if device.type=='cuda' else 'cpu', enabled=False):
            ...
            # L2 Prior
            prior_term_l2 = y_hat_32.pow(2).mean() if prior_l2 > 0.0 else y_hat_32.new_tensor(0.0)

            # Soft Boundary Prior
            prior_term_bnd = NormCalc_prior_bnd(device, y_tf, y_hat_32, PARAM_RANGE, prior_bound, prior_bound_margin )
            ...
            if (proxy_g is not None and lambda_cyc_sim > 0.0):
                ...
                # cyc_sim = NormCalc_cyc(device, proxy_g, lambda_cyc_sim, y_tf, y_tf_proxy, y_hat_32,
                #         x, x_mu_c, x_std_c, x_mu_p, x_std_p, y_idx_c_from_p, cyc_crit)

                # FIXME: use prior rather than posterior for cycle-sim
                cyc_sim, x_hat_std_sim_prior = NormCalc_cyc(device, proxy_g, lambda_cyc_sim, 
                                    y_tf, y_tf_proxy, y_hat_prior.to(torch.float32),
                                    x, x_mu_c, x_std_c, x_mu_p, x_std_p, 
                                    y_idx_c_from_p, cyc_crit)
                loss += lambda_cyc_sim * cyc_sim

            # Cycle-Meas Consistency
            cyc_meas = y_hat_32.new_tensor(0.0)
            if (proxy_g is not None and meas_iter is not None and lambda_cyc_meas > 0.0):
                ...
                # --- FIX START: Switch to eval mode for measurement data ---
                was_training = model.training
                if was_training:
                    model.eval()

                # Use prior for inference on measurement data
                ym_hat, _, _ = model(xm)
                ...
                if ((cyc_meas_knn_weight or (trust_alpha_meas > 0.0)) and (yref_proxy_norm is not None)):
                    ...
                    dists_m = torch.cdist(ym_proxy_norm, yref_sub, p=2)
...
def evaluate_full(model, loader, device,
    ...
    if (proxy_g is not None and meas_loader is not None and lambda_cyc_meas > 0.0):
        for xm in meas_loader:
            xm = xm.to(device, non_blocking=True)
            # Use prior network for inference as no 'y' is available
            ym_hat, _, _ = model(xm)
            cyc_meas, x_hat_std_meas = NormCalc_cyc(device, proxy_g, lambda_cyc_meas, y_tf, y_tf_proxy, ym_hat,
                        xm, x_mu_c, x_std_c, x_mu_p, x_std_p, y_idx_c_from_p, cyc_crit)

            meter['cyc_meas'] += cyc_meas.item() * xm.size(0)
            meter['n_meas']  += xm.size(0)

            # diagnostics recording
            domain_label = 'meas'
            y_phys_m = y_tf.inverse(ym_hat.to(torch.float32))
            ...
            diag_rows, diag_count = diag_processing(
                        ...
                        domain_label, 
                        diag_rows, diag_count, diag_max, diag_k,
                        xm, xhat_m_std, x_mu_p, x_std_p, x_mu_c, x_std_c,
                        y, ym_hat, ym_hat.to(torch.float32), y_tf, y_tf_proxy, y_idx_c_from_p, yref_proxy_norm,
                        prior_bound, prior_bound_margin, PARAM_RANGE,
                        proxy_floor_all=proxy_floor_all
                        ) if diag_enabled and (y_tf is not None) and (y_tf_proxy is not None) and (proxy_g is not None) and (diag_tag == "test") else (diag_rows, diag_count)
